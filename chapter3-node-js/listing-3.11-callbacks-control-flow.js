...
function getTemperature(callback) {
  request({url: serviceRootUrl + '/temperature', json: true}, function (err, res, body) {
    ...
    if (res && res.statusCode === 200) {
      ...
      callback(null, temp); //#D Call the next function in the series
...
function getLight(callback) {
  request({url: serviceRootUrl + '/light', json: true}, function (err, res, body) {
    ...
    if (res && res.statusCode === 200) {
      ...
      callback(null, light); //#D Call the next function in the series
    } else callback(null, null);
  });
}
...
http.createServer(function (req, res) {
  ...
  if (req.url === '/log') {
    async.series([            //#A We create an array of functions to be invoked in series
        getTemperature,
        getLight
      ],
      function (err, results) { //#B This function is called when the last function in the series returned
        console.log(results);   //#C results is now equal to [light, temperature]
